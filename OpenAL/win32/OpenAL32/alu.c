...
ALUAPI ALvoid ALUAPIENTRY aluCalculateSourceParameters(ALcontext* context, ALuint source, ALuint channels, ALfloat* drysend, ALfloat* wetsend, ALfloat* pitch)
{
    ...
    if(alIsSourceSafe(context, source))
    {
        //Get global properties
        DopplerFactor = context->DopplerFactor;
        DistanceModel = context->DistanceModel;
        //Get source properties
        alSource = (ALsource*)source;
        Pitch = alSource->param[AL_PITCH-AL_CONE_INNER_ANGLE].f;
        Volume = alSource->param[AL_GAIN-AL_CONE_INNER_ANGLE].f;
        MinVolume = alSource->param[AL_MIN_GAIN-AL_CONE_INNER_ANGLE].f;
        MaxVolume = alSource->param[AL_MAX_GAIN-AL_CONE_INNER_ANGLE].f;
        MinDist = alSource->param[AL_REFERENCE_DISTANCE-AL_CONE_INNER_ANGLE].f;
        MaxDist = alSource->param[AL_MAX_DISTANCE-AL_CONE_INNER_ANGLE].f;
        Rolloff = alSource->param[AL_ROLLOFF_FACTOR-AL_CONE_INNER_ANGLE].f;

        Position[0] = alSource->param[AL_POSITION-AL_CONE_INNER_ANGLE].fv3[0];
        Position[1] = alSource->param[AL_POSITION-AL_CONE_INNER_ANGLE].fv3[1];
        Position[2] = alSource->param[AL_POSITION-AL_CONE_INNER_ANGLE].fv3[2];

        Velocity[0] = alSource->param[AL_VELOCITY-AL_CONE_INNER_ANGLE].fv3[0];
        Velocity[1] = alSource->param[AL_VELOCITY-AL_CONE_INNER_ANGLE].fv3[1];
        Velocity[2] = alSource->param[AL_VELOCITY-AL_CONE_INNER_ANGLE].fv3[2];

        Direction[0] = alSource->param[AL_DIRECTION-AL_CONE_INNER_ANGLE].fv3[0];
        Direction[1] = alSource->param[AL_DIRECTION-AL_CONE_INNER_ANGLE].fv3[1];
        Direction[2] = alSource->param[AL_DIRECTION-AL_CONE_INNER_ANGLE].fv3[2];

        OuterGain = alSource->param[AL_CONE_OUTER_GAIN-AL_CONE_INNER_ANGLE].f;
        InnerAngle = alSource->param[AL_CONE_INNER_ANGLE-AL_CONE_INNER_ANGLE].f;
        OuterAngle = alSource->param[AL_CONE_OUTER_ANGLE-AL_CONE_INNER_ANGLE].f;

        HeadRelative = alSource->relative;

        //Get listener properties
        ListenerGain = context->Listener.Gain;
        ListenerPosition[0] = context->Listener.Position[0];
        ListenerPosition[1] = context->Listener.Position[1];
        ListenerPosition[2] = context->Listener.Position[2];
        ListenerVelocity[0] = context->Listener.Velocity[0];
        ListenerVelocity[1] = context->Listener.Velocity[1];
        ListenerVelocity[2] = context->Listener.Velocity[2];
        ListenerOrientation[0] = context->Listener.Forward[0];
        ListenerOrientation[1] = context->Listener.Forward[1];
        ListenerOrientation[2] = context->Listener.Forward[2];
        ListenerOrientation[3] = context->Listener.Up[0];
        ListenerOrientation[4] = context->Listener.Up[1];
        ListenerOrientation[5] = context->Listener.Up[2];
    ...
    if(context)
    {
        ...
        if(buffer && size)
        {
            ...
            for(i=0;i<alContext->SourceCount;i++)
            {
                ...
                {
                    aluCalculateSourceParameters(context, (ALuint)alSource,OUTPUTCHANNELS,DrySend,WetSend,&Pitch);
                    //Get buffer info
                    Buffer = alSource->param[AL_BUFFER-AL_CONE_INNER_ANGLE].i;
                    if(Buffer)
                    {
...
//                                              BufferSize=(ALuint)(((ALfloat)(DataSize-DataPosInt))/Pitch)+1;
                    ...
                    if((!Buffer)||(DataPosInt>=DataSize))
                    {
                        ...
                        if(alSource->queue)
                        {
                            Looping = alSource->param[AL_LOOPING-AL_CONE_INNER_ANGLE].i;
                            if(alSource->BuffersAddedToDSBuffer < (alSource->BuffersInQueue-1))
                            {
                                ...
                                if(BufferListItem)
                                    alSource->param[AL_BUFFER-AL_CONE_INNER_ANGLE].i=BufferListItem->buffer;
                                alSource->position=0;
