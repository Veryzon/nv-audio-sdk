// nvwvfmt.cpp : Defines the entry point for the console application.
//
...
FORMAT_TAG_STR FormatTagStr[] =
{
    ...
    {"WAVE_FORMAT_EXTENSIBLE",                 0xFFFE}, /* FAST Multimedia AG */
    {"WAVE_FORMAT_TEST",                       0xFFFF}, /* FAST Multimedia AG */
    {0,                                        0x0000}
};
...
int main(int argc, char* argv[])
{
    ...
    {
        printf("nvwvfmt.exe\n\n");
        printf("    Description: Wave file header viewer.\n");
        printf("    Syntax:      nvwvfmt [/d] <filename.wav>\n\n");
        printf("    Options:\n");
        printf("        [/d]     Dumps the wave file data.\n");
        printf("\n");
        return 0;
    }
    ...
    CWaveFile file;
    file.LoadFile(argv[name]);
    printf("Filename: %22s\n", argv[name]);
    printf("File Size:            %10d (0x%08x)\n", file.GetFileSize(), file.GetFileSize());
    printf("\n");
    printf("Format Information:\n");
    printf("    Size:             %10d (0x%08x)\n", file.GetFormatSize(), file.GetFormatSize());
    printf("    Tag:                  0x%04x ", file.GetWaveFormat());

    DWORD i = 0;
    while(FormatTagStr[i].Str)
    {
        if(FormatTagStr[i].Tag == file.GetWaveFormat())
        {
            printf("%s\n", FormatTagStr[i].Str);
            break;
        }
    ...
    if(!FormatTagStr[i].Str)
    {
        printf("UNKNOWN TAG\n");
    }

    printf("    SampleRate:       %10d (0x%04x)\n", file.GetSampleRate(), file.GetSampleRate());
    printf("    Bits Per Sample:  %10d\n", file.GetBitsPerSample());
    printf("    Channels:         %10d\n", file.GetChannels());
    printf("    Avg bytes/second: %10d\n", file.GetAvgBytesPerSec());
    printf("    Block Alignment:  %10d\n", file.GetBlockAlign());
    printf("\n");

    switch(file.GetWaveFormat())
    {
        case WAVE_FORMAT_EXTENSIBLE:
        {
            printf("Additional Format Information:\n");
            printf("    Subformat:       %08x-%04x-%04x-%02x%02x%02x%02x%02x%02x%02x%02x\n",
                   file.GetSubFormat().Data1, file.GetSubFormat().Data2, file.GetSubFormat().Data3,
                   file.GetSubFormat().Data4[0], file.GetSubFormat().Data4[1],
                   file.GetSubFormat().Data4[2], file.GetSubFormat().Data4[3],
                   file.GetSubFormat().Data4[4], file.GetSubFormat().Data4[5],
                   file.GetSubFormat().Data4[6], file.GetSubFormat().Data4[7]);
        }
        break;
        ...
        {
            printf("Additional Format Information:\n");
            IMAADPCMWAVEFORMAT* fmt = (IMAADPCMWAVEFORMAT*)file.GetFormatBuffer();
            printf("    Samples/Block:    %10d\n", fmt->wSamplesPerBlock);
        }
        break;
    ...
    if(dumpWaveData)
    {
        ...
        for(i = 0; i < file.GetDataSize(); )
        {
            ...
            c = (int)((char*)file.GetDataBuffer())[i];
            if(c < 0) c = 256 + c;
            printf("%02x", c);
            i++;
        }
