//************************************************************************
// Simple6Chand3D.cpp : A simple program for experimenting with nVidia 
//                      6 channel audio and 3D audio simultaneously.
//
//************************************************************************
                     ...
                     int       nCmdShow)
{
    ...
    HWND hDlg = CreateDialog(g_hInst, (LPCTSTR)IDD_SIMPLE6CHAND3D, NULL, (DLGPROC)DialogMsgHandler );
    if (!hDlg)
    {
        return FALSE;
    }
	...
	hAccelTable = LoadAccelerators(hInstance, (LPCTSTR)IDA_SIMPLE6CHAND3D);
	...
	switch (message) 
	{
		case WM_COMMAND:
			 ...
			 switch (wmId)
             {
                 ...
                 case IDC_OPEN1:
                      OpenFileToPlay( hDlg, 0, NULL, FALSE );
                      break;

                 case IDC_OPEN2:
                      OpenFileToPlay( hDlg, 1, NULL, TRUE );
                      break;
...
bool InitializeDialog( HWND hDlg )
{
    ...
    if(g_pSoundManager == NULL)
    {
        ...
        MessageBox( hDlg, "Error creating Sound manager.  Program will now exit.", 
                    "nVidia SIMPLE6CHAND3D", MB_OK | MB_ICONERROR );
        DestroyWindow( hDlg );
        return FALSE;
    }
    ...
    if(g_pCOrbiter)
    {
        g_pCOrbiter->Initialize( hDlg, g_pKsPropSet, g_pDSListener );
    }
...
bool OpenFileToPlay( HWND hDlg, DWORD FileNum, TCHAR * pFilename, bool Is3D )
{
    ...
    if(Is3D)
    {
        if( FAILED( Validate3DWaveFile( hDlg, g_Buffer[FileNum].strFileName, g_Buffer[FileNum].FileControl ) ) )
        {
            // Set the UI controls
            SetDlgItemText( hDlg, g_Buffer[FileNum].FileControl, TEXT("") );
            return S_FALSE;
    ...
    {
        if( FAILED( ValidateExtensibleWaveFile( hDlg, g_Buffer[FileNum].strFileName, g_Buffer[FileNum].FileControl ) ) )
        {
            // Set the UI controls
            SetDlgItemText( hDlg, g_Buffer[FileNum].FileControl, TEXT("") );
            return S_FALSE;
    ...
    //
    g_Buffer[FileNum].pDSB = g_Buffer[FileNum].pSound->GetBuffer( 0 );
    if( g_Buffer[FileNum].pDSB == NULL )
    {
    ...
    g_Buffer[FileNum].Is3D = Is3D;

    //
    // If other buffers are already playing, lets play this buffer right now.
    //
    PlayIfOthersPlaying( hDlg, FileNum );
...
bool StartPlayingWave( HWND hDlg )
{
    ...
    for(i = 0; i < MAX_FILES; i++)
    {
        ...
        {
            // Send the Orbiter the 3D buffer pointer
            if(g_Buffer[i].Is3D)
            {
                g_pCOrbiter->SetDSBuffer( g_Buffer[i].pDSB );
            }
...
bool StopPlayingWave( HWND hDlg )
{
    ...
    for(i = 0; i < MAX_FILES; i++)
    {
        if(g_Buffer[i].Active)
        {
            ...
            // Zero the 3D buffer pointer in the Orbiter
            if(g_Buffer[i].Is3D)
            {
                g_pCOrbiter->SetDSBuffer( 0 );
            }
        }
    }
...
//************************************************************************
// Name: PlayIfOthersPlaying
// Desc: If we are already playing, begin playing non-playing buffers.
//************************************************************************
bool PlayIfOthersPlaying( HWND hDlg, DWORD BufferNum )
{
    ...
    // Send the Orbiter the 3D buffer pointer
    if(g_Buffer[BufferNum].Is3D)
    {
        g_pCOrbiter->SetDSBuffer( g_Buffer[BufferNum].pDSB );
    }
...
//************************************************************************
// End of File: Simple6Chand3D.cpp
//************************************************************************
