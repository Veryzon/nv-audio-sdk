...
//
// Portions Copyright(c) 2000-2002 NVIDIA Corporation. All rights reserved.
//
//-----------------------------------------------------------------------------
#define STRICT
#include <windows.h>
                               ...
                               DWORD dwNumBuffers )
{
    ...
    ZeroMemory( &dsbd, sizeof(DSBUFFERDESC) );
    dsbd.dwSize          = sizeof(DSBUFFERDESC);
    dsbd.dwFlags         = dwCreationFlags | DSBCAPS_GLOBALFOCUS ;
    ...
    if( FAILED(hr) )
    {
		...
		// We do this now in the main program, not here! JDW
...
//-----------------------------------------------------------------------------
// Name: CSound::GetKsPropSetInterface()
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CSound::GetKsPropSetInterface( DWORD dwIndex, LPKSPROPERTYSET* ppKsPropertySetPath )
{
    if( m_apDSBuffer == NULL )
        return CO_E_NOTINITIALIZED;
    if( dwIndex >= m_dwNumBuffers )
    ...
    *ppKsPropertySetPath = NULL;

    return m_apDSBuffer[dwIndex]->QueryInterface( IID_IKsPropertySet, 
                                                  (VOID**)ppKsPropertySetPath );
}
...
//-----------------------------------------------------------------------------
// Name: CSound::GetFrequency
// Desc: Return the frequency
//-----------------------------------------------------------------------------
DWORD CSound::GetFrequency()
{
    if(m_pWaveFile)
    {
        WAVEFORMATEX* pwfx = m_pWaveFile->m_pwfx;
...
//-----------------------------------------------------------------------------
// Name: CSound::GetChannels
// Desc: Return the number of channels...mono(1), stereo(2) etc....
//-----------------------------------------------------------------------------
DWORD CSound::GetChannels()
{
    if(m_pWaveFile)
    {
        WAVEFORMATEX* pwfx = m_pWaveFile->m_pwfx;
...
HRESULT CWaveFile::Open( LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
        ...
        if( NULL == m_hmmio )
        {
            ...
            SAFE_DELETE_ARRAY( pData );  // Added JDW
        }
