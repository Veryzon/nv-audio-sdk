...
COrbiter::COrbiter( )
{
    ...
    m_pKsPropSet  = NULL;
    m_pDSListener = NULL;
    m_FxManagerId.FxManagerId = NULL;
    m_FxSubmixId  = NULL;
...
( 
    HWND                        hDlg,
    NVFXINST_FXMANAGER_ID       FxManagerId,
...
//************************************************************************
// Name: SetSubmixId
// Desc: Load the SubmixId and enable orbiting if Id not zero.
//************************************************************************
VOID COrbiter::SetSubmixId(NVFXDATA_FX_ID SubmixId) 
{ 
    m_FxSubmixId = SubmixId; 
...
VOID COrbiter::SetObjectProperties( DS3DVECTOR* pvPosition, DS3DVECTOR* pvVelocity )
{
    ...
    //
    // Dont start sending data until the Submix ID is valid.
    //
    if(m_FxSubmixId)
    {
        HRESULT hr;
        if( FAILED( hr = Set3DSubmixParameters(m_pKsPropSet, &m_FxManagerId, m_FxSubmixId,
                              KSPROPERTY_DIRECTSOUND3DBUFFER_POSITION, 
                              sizeof(DS3DVECTOR), &m_DsBufferParams.vPosition )))
        {
            m_bAllowMovementTimer = FALSE;
            return;
        }
        ...
        if( FAILED( hr = Set3DSubmixParameters(m_pKsPropSet, &m_FxManagerId, m_FxSubmixId,
                              ...
                              sizeof(DS3DVECTOR), &m_DsBufferParams.vVelocity )))
        {
            m_bAllowMovementTimer = FALSE;
            return;
        }
